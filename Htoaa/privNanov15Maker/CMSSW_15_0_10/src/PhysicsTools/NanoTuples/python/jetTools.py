##################################################
# Adapted from https://github.com/cms-sw/cmssw/blob/CMSSW_15_0_10/PhysicsTools/PatAlgos/python/tools/jetTools.py
#   - include custom taggers (annotated with !!!)
##################################################
...
###!!! custom
from PhysicsTools.NanoTuples.newTagger.bTaggingCustomUtils import updateSupportedBtagDiscr
...
def setupBTagging(process, jetSource, pfCandidates, explicitJTA, pvSource, svSource, elSource, muSource, runIVF, tightBTagNTkHits, loadStdRecoBTag, svClustering, fatJets, groomedFatJets,
    ...
    import PhysicsTools.NanoTuples.newTagger.bTaggingCustomUtils as btag_utils
...
#                     vertex_associator = "primaryVertexAssociation:original"
            ...
            if btagInfo in ['pfMassDecorrelatedDeepHWWV1TagInfos', 'pfMassDecorrelatedDeepHggV3TagInfos', 'pfMassDecorrelatedInclParticleTransformerV1TagInfos', 'pfMassDecorrelatedInclParticleTransformerV2TagInfos', 'pfMassDecorrelatedInclParticleTransformerAK15V2TagInfos']:
                if pfCandidates.value() == 'packedPFCandidates':
                    ...
                    raise ValueError("Invalid pfCandidates collection: %s." % pfCandidates.value())
                # btagInfoModule = getattr(__import__("PhysicsTools.NanoTuples.newTagger.%s_cff" % btagInfo.replace("TagInfos", ""), fromlist=[btagInfo]), btagInfo)
                addToProcessAndTask(btagPrefix+btagInfo+labelName+postfix,
