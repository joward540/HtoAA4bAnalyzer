...
./worker_privSig_GENtoprivNanov15.sh -nanogen ${model}_fragment.py $nEvents
if [ $? == 0 ] ; then
...
exitCode=0
mv nanoGENFile.root PrivSample_${model}_NanoGEN_job${jobID}.root

echo "copying nanoGEN......"
if repeated_copy PrivSample_${model}_NanoGEN_job${jobID}.root $outFileLoc 5; then
